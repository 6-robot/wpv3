...
float CWPV3_driver::m_CalQuaternionVal(unsigned char *inBuf)
{
...
void CWPV3_driver::Parse(unsigned char inData)
{
	m_nRecvByteCnt ++;
...
void CWPV3_driver::m_ParseFrame(unsigned char *inBuf, int inLen)
{
	int i;
	switch (inBuf[2])		//ID
	{
...
void CWPV3_driver::Velocity(float inX, float inY, float inAngular)
{
	//upward backward
...
void CWPV3_driver::m_GenerateSigCmd(int inIndex, unsigned char inID, unsigned char inLen, unsigned char inMode, unsigned char inMethod, unsigned char *data)
{
...
unsigned char CWPV3_driver::m_CalSum(int length)
{
	int temp = 0;
